...
<%
	...
	ArrayList<NoticeDto> list = dao.select(start, displayCount, searchType, searchText); //
%>
    <!-- container start -->
    <div class="container">
      <!-- row1 start -->
        ...
        <div class='float-lg-right'>
          <form class="form-inline" method="post" name="f" action="notice_search.jsp">
            <select name="searchType" id="searchType" class="form-control my-2 mr-lg-2">
			  <option value="n_title" <%if(searchType.equals("n_title")) out.print("selected"); %>>제목</option>
			  <option value="n_content" <%if(searchType.equals("n_content")) out.print("selected"); %>>내용</option>
			</select>
		    <input class="form-control mr-lg-2" type="search" placeholder="Search" name="searchText" value="<%=searchText %>" id="searchText" aria-label="Search">
      ...
      <div class="row">
        ...
        <div class="col-lg-12">
         <div class='float-lg-right mt-2'>
          ...
          <a href="list_notice.jsp?page=<%=cPage %>" class="btn btn-outline-secondary">목록</a>
          </div>
        </div>
        <!-- col end -->
      </div>
      ...
      <%
      	...
      	int totalRows = dao.getRows(searchType, searchText);//128
        ...
        <div class="col-lg-12">
          <nav aria-label="Page navigation example">
            <ul class="pagination justify-content-center">
             ...
             <%}else{ %>
             <li class="page-item"><a class="page-link" href="notice_search.jsp?page=<%=startPage-1%>&searchType=<%=searchType%>&searchText=<%=searchText%>">&laquo;</a></li>
             <%} %> 
              <%for(int i=startPage;i<=endPage;i++){ %>
              <li class="page-item"><a class="page-link" href="notice_search.jsp?page=<%=i%>&searchType=<%=searchType%>&searchText=<%=searchText%>"><%=i %></a></li>
              <%} %>
              <%if(currentBlock == totalBlock){ %>
              <li class="page-item"><a class="page-link" href="#">&raquo;</a></li>
              <%}else{ %>
              <li class="page-item"><a class="page-link" href="notice_search.jsp?page=<%=endPage+1%>&searchType=<%=searchType%>&searchText=<%=searchText%>">&raquo;</a></li>
              <%} %>
